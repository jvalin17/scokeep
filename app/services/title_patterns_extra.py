    ...
    for player in ctx.players:
        ...
        if total >= 2 and made > 0:
            ...
            out.append(_candidate(
                "trump_master", "♠️", "Trump Master", "Best accuracy on 6-8 card rounds",
                player, f"{made} of {total} bids correct on high-card rounds", pct * 100,
            ))
    return out
...
def _minimalist(ctx: GameContext) -> list[dict]:
    ...
    for player in ctx.players:
        ...
        if total >= 2:
            pct = made / total
            out.append(_candidate(
                "minimalist", "✨", "Minimalist", "Best accuracy on 1-3 card rounds",
                player, f"{made} of {total} bids correct on low-card rounds", pct * 100,
            ))
    return out
...
def _mirror(ctx: GameContext) -> list[dict]:
    ...
    for i, player_a in enumerate(ctx.players):
        for j, player_b in enumerate(ctx.players):
            ...
            shared = sum(
                1 for s1, s2 in zip(
                    ctx.round_scores[player_a], ctx.round_scores[player_b], strict=False,
                ) if s1 == s2
            )
            if shared >= 3:
                out.append(_candidate(
                    "mirror", "🪞", "Mirror", f"Same score as {player_b} in 3+ rounds",
                    player_a, f"{shared} matching rounds", 35,
                ))
                out.append(_candidate(
                    "mirror", "🪞", "Mirror", f"Same score as {player_a} in 3+ rounds",
                    player_b, f"{shared} matching rounds", 35,
                ))
    return out
...
def _lucky_seven(ctx: GameContext) -> list[dict]:
    ...
    for player in ctx.players:
        ...
        if count >= 7:
            out.append(_candidate(
                "lucky_seven", "🍀", "Lucky Seven", "Scored +10 or +11 in 7+ rounds",
                player, f"{count} rounds", 45,
            ))
    return out
...
def _last_laugh(ctx: GameContext) -> list[dict]:
    ...
    if worst_first == best_second:
        out.append(_candidate(
            "last_laugh", "😏", "Last Laugh", "Worst 1st half, best 2nd half",
            worst_first, f"{first_half[worst_first]}→{second_half[worst_first]}", 50,
        ))
    return out
...
def _survivor(ctx: GameContext) -> list[dict]:
    ...
    for player in ctx.players:
        ...
        if played_all and ctx.totals[player] > 0 and all(s < 20 for s in scores):
            out.append(_candidate(
                "survivor", "🛟", "Survivor", "Played all rounds, positive total, no huge score",
                player, f"{ctx.totals[player]} pts steady", 15,
            ))
    return out
