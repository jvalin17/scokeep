...
async def _setup(client: AsyncClient, name: str, players: list[str]):
    ...
    await client.post("/api/playground", json={
        "name": name, "pin": "1234", "players": players,
    })
...
class TestBidEditViaPatch:
    ...
    async def test_edit_multiple_players_then_start(self, client: AsyncClient):
        ...
        pg, cookies = await _setup(
            client, "Edit Multiple", ["Alice", "Bob", "Charlie"],
        )
        ...
        await client.patch(f"/api/game/{game_id}/bid/0", json={
            "value": 4,
        }, cookies=cookies)
        await client.patch(f"/api/game/{game_id}/bid/2", json={
            "value": 0,
        }, cookies=cookies)
...
class TestMustLoseWithEdits:
    ...
    async def test_must_lose_blocks_forbidden_value(self, client: AsyncClient):
        ...
        pg, cookies = await _setup(
            client, "Must Lose Basic", ["Alice", "Bob", "Charlie"],
        )
        game = await _create_game(
            client, pg["id"], cookies,
            ["Alice", "Bob", "Charlie"], must_lose=True,
        )
        game_id = game["id"]
    ...
    async def test_edit_shifts_forbidden_value(self, client: AsyncClient):
        ...
        pg, cookies = await _setup(
            client, "Must Lose Shift", ["Alice", "Bob", "Charlie"],
        )
        game = await _create_game(
            client, pg["id"], cookies,
            ["Alice", "Bob", "Charlie"], must_lose=True,
        ...
        await client.patch(f"/api/game/{game_id}/bid/1", json={
            "value": 4,
        }, cookies=cookies)

        # Now Charlie's bid of 1 makes total = 3+4+1 = 8 = cards dealt
        # Charlie needs to change — edit to 0 (total = 7)
        resp = await client.patch(f"/api/game/{game_id}/bid/2", json={
            "value": 0,
        }, cookies=cookies)
        assert resp.status_code == 200
    ...
    async def test_edit_non_last_player_no_restriction(self, client: AsyncClient):
        ...
        pg, cookies = await _setup(
            client, "Must Lose Non-Last", ["Alice", "Bob", "Charlie"],
        )
        game = await _create_game(
            client, pg["id"], cookies,
            ["Alice", "Bob", "Charlie"], must_lose=True,
...
class TestBidNavigationFlow:
    ...
    async def test_submit_go_back_change_go_forward(self, client: AsyncClient):
        ...
        pg, cookies = await _setup(
            client, "Nav Flow", ["Alice", "Bob", "Charlie"],
        )
        game = await _create_game(
            client, pg["id"], cookies, ["Alice", "Bob", "Charlie"],
        )
