...
def test_dialog_closes_on_overlay_click(stats_with_game):
    ...
    page.locator(".action-overlay").wait_for(state="hidden", timeout=3000)
...
def test_edit_mode_toggle(stats_with_game):
    ...
    page.wait_for_function("() => document.querySelector('.edit-mode') !== null", timeout=5000)
...
def test_edit_mode_scores_visible(stats_with_game):
    ...
    if games_tab.count() > 0:
        games_tab.click()
        page.wait_for_selector(".stats-content, .stats-game-card", timeout=5000)
...
def test_clear_stats_requires_delete_confirmation(stats_with_game):
    ...
    page.wait_for_function(
        "() => !document.querySelector('#clear-proceed')?.disabled",
        timeout=3000,
    )
...
def test_clear_stats_cancel(stats_with_game):
    ...
    page.locator(".clear-warning").wait_for(state="hidden", timeout=3000)
