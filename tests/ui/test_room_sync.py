...
def test_room_list_shows_cached_rooms(room_page):
    ...
    room_page.click('.tab[data-tab="quick"]')
    room_page.wait_for_selector("#quick-form:not(.hidden)", timeout=3000)

    room_list = room_page.locator("#quick-room-list")
...
def test_pin_input_appears_on_room_click(room_page):
    ...
    room_page.locator(".quick-room-item").first.click()

    pin_input = room_page.locator("#quick-room-pin")
    expect(pin_input).to_be_visible(timeout=3000)

    # Selected room name should be shown
    selected_label = room_page.locator("#quick-room-selected")
...
def test_correct_pin_autofills_players(room_page):
    ...
    room_page.locator(".quick-room-item").first.click()
    room_page.wait_for_selector("#quick-room-pin", timeout=3000)

    room_page.fill("#quick-room-pin", "1234")
